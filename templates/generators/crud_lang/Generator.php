...
namespace cza\base\templates\generators\crud_lang;
...
class Generator extends \yii\gii\Generator {
    ...
    public $baseControllerClass = '\cza\base\components\controllers\backend\Controller';
    ...
    public function getName() {
        return 'Cza CRUD With Languages';
    }
    ...
    public function generate() {
        ...
        // add widget views
        $widgetViewsPath = $this->getWidgetsPath() . '/views';
        $templatePath = $this->getTemplatePath() . '/widgets/views';
        foreach (scandir($templatePath) as $file) {
            if (is_file($templatePath . '/' . $file) && pathinfo($file, PATHINFO_EXTENSION) === 'php') {
                $files[] = new CodeFile("$widgetViewsPath/$file", $this->render("widgets/views/$file"));
            }
        }
    ...
    /**
     * @return string the action widgets file path
     */
    public function getWidgetsPath() {
        $module = empty($this->moduleID) ? Yii::$app : Yii::$app->getModule($this->moduleID);

        return $module->getBasePath() . '/widgets';
    }
    ...
    public function generateActiveField($attribute) {
        ...
        if(in_array($attribute, ['created_by', 'updated_by', 'self_status', 'created_at', 'updated_at'])){
            return "";
        }
        
        if($attribute == 'priority'){
            return "\$form->field(\$model, '$attribute')->spinnerInput()";
        }
    ...
    /**
     * Generates code for language active field
     * @param string $attribute
     * @return string
     */
    public function generateLanguageActiveField($attribute) {
        $tableSchema = $this->getTableSchema();
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            if (preg_match('/^(password|pass|passwd|passcode)$/i', $attribute)) {
                return "\$form->field(\$model, '$attribute')->passwordInput()";
